    ...
    for (let i = 0; i <= 2; i++) {
        await producer.send({
            ...
            messages: [
                {
                    // Vùng muốn gửi
                    partition: i, // Phân vùng cụ thể trong chủ đề nơi tin nhắn sẽ được gửi. Nếu không được chỉ định, Kafka sẽ xác định phân vùng dựa trên khóa hoặc sử dụng chiến lược phân vùng mặc định.
